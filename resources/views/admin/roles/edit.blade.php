...
@section('content')
    <div class="mx-auto max-w-3xl">
        <header>
            ...
            <h1 class="mt-2 text-3xl font-bold tracking-tight">Editar rol</h1>
        </header>

        <form method="POST" action="{{ route('admin.roles.update', $rol) }}" class="mt-8">
            @csrf
            @method('PUT')
            @include('admin.partials.form-rol', ['rolForm' => $rol])
            @include('admin.partials.checkboxes-permisos', ['permisosSeleccionados' => $rol->permissions->pluck('id')->toArray()])
